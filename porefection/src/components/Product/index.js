...
function Product({ index, routine, getRoutine, product }) {
	const handleAccordian = (e) => {
		...
		carrot.style.transform === 'scaleY(-1)' ? (carrot.style.transform = 'scaleY(1)') : (carrot.style.transform = 'scaleY(-1)');

		// close all other panels
		let accordian = document.getElementsByClassName('accordian');

		const closeAllPanels = () => {
			for (let menu of accordian) {
			...
			<div className='product-details'>
						...
						.map((line, index) => {
							return (
								<li key={index} className='description-line'>
									{line
										.replace(/What it is/g, '')
										.split(':')
										.map((item, i) => {
											return (
												<p key={i} className='description-item' style={i === 0 ? { fontWeight: '500' } : {}}>
													{item}
												</p>
											);
										})}
				...
				<ol className='accordian suggested-use'>
					{product.suggestedUsage
						.replace(/<br>|<b>|<\/b>|<p>|<\/p>/g, '')
						.replace(/&rsquo;/g, `'`)
						.split('-')
						.map((line, index) => {
							return (
								index > 0 && (
									<li key={index} className='use-line'>
										<p>{line}</p>
									</li>
								)
							);
				...
				<ul className='accordian ingredient-breakdown'>
					{product.currentSku.ingredientDesc
						.split('Clean at Sephora')[0]
						.replace(/, Inactive Ingredients|, Inactive Ingredients<br>/g, '<br>')
						.replace(/<br><\/b>/g, ':')
						.replace(/<b>|<\/b>|<p>|<\/p>| &ndash/g, '')
						.split('<br>')
						.map((line, index) => {
							while (line.charAt(0) === '-') {
								...
								line && (
									<div className='ingredient'>
										{line.split(':').map((item, i) => {
											return (
												<p key={index + '.' + i} className='ingredient-line'>
													{item}
												</p>
											);
										})}
